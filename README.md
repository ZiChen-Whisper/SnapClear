...
SnapClear 不只是一个可以在 OPPO 手机上运行的通用 Android 应用，而是围绕 ColorOS 的后台机制、系统通知和交互动效进行专项调优：

- **OPPO 后台调优**：针对 ColorOS 的 Hans/Athena 进程冻结、后台回调延迟和自启动限制，组合无障碍事件唤醒、MediaStore 变化、文件事件、前台服务和系统调度任务，尽可能降低后台截图漏检。
- **ColorOS 流体云**：在 Android 16 / ColorOS 16 及以上系统中接入 Live Updates。检测到截图后，流体云可展示截图状态、倒计时，以及“拷贝并删除”“忽略”等快捷操作；未授权或系统不支持时自动回退为高优先级通知。
- **OPPO 无缝动画**：截图卡片、详情页和关于页优先使用 OPPO View 无缝过渡，让卡片内容自然衔接到新页面。该能力需要 ColorOS 16.1+ 且机型动效等级达到 B+；不支持的设备会自动回退到普通页面动画。
- **沉浸式顶栏与小白条**：状态栏和导航栏均采用透明、边到边布局，内容和渐变延伸到系统栏区域，同时正确避让状态栏、底部手势区域与小白条。
- **安卓原生侧滑跟手返回**：支持 Android 原生预测性返回动画。在 ColorOS 16 及以上设备中，可在系统设置里开启相关返回动画选项，体验手势进度与页面动画同步的侧滑跟手返回。

## 主要功能

- 后台持续监听新截图，并通过多层检测机制降低漏检概率
- 通知快捷操作：拷贝并删除、忽略
- Android 16 / ColorOS 16 流体云（Live Updates）通知与倒计时
- 最近截图卡片与列表视图、分页浏览和截图详情
- OPPO 权限页面引导、电池优化与精确闹钟状态检查
- 深色模式、沉浸式系统栏、原生侧滑返回与自适应应用图标
- 开机后恢复已启用的截图监听
...
- 流体云与原生侧滑跟手返回的完整体验推荐 ColorOS 16 及以上；OPPO 无缝动画要求 ColorOS 16.1+ 和受支持的动效等级
